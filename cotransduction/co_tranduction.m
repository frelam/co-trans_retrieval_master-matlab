function [Results] = co_transduction (Diff_1, Diff_2, Co_num,item)
...
Neighbor=10;
   ...
   for k=1:WINDOWS% calculate sigma for affinity matrix
       for j=1:WINDOWS
           SIGMA=mean([TT_1(k,2:Neighbor),TT_1(j,2:Neighbor)]);
           W(k,j)=normpdf(Real_Diff_1(k,j),0,K*SIGMA);
       end
   end
   [m,n] = size(W);
   ...
   for k=1:WINDOWS% calculate sigma for affinity matrix
       for j=1:WINDOWS
           SIGMA=mean([TT_2(k,2:Neighbor),TT_2(j,2:Neighbor)]);
           W(k,j)=normpdf(Real_Diff_2(k,j),0,K*SIGMA);
       end
   end
   [m,n] = size(W);
   Norm = repmat(sum(W')',1,n);
